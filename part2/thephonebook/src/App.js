...
const App = () => {
  ...
  const updateNumber = (actuelContact, updateContact) => {
    if (window.confirm(`${actuelContact.name} is already added to the phonebook, replace the old number with a new one?`)){
      ...
      .catch(error => {
        setErrorMessage (
          `ERROR: ${actuelContact.name} has not been updated`
        )
        setTimeout(() => {
  ...
  const addContact = (event) => {
    ...
    if (isPresent){
      ...
      if (newPerson.name === '' || newPerson.number === ''){
        ...
        .then(newContact => {
          console.log (`newcontact ${newContact.id} ${newContact.name}`)
        ...
        .catch(error => {
          console.log (error)
          setErrorMessage(
          `ERROR: Creation Contact: ${newPerson.name} Failed`
          )
          setTimeout(() => {
            setErrorMessage(null)}, 5000)
          setNewPerson({
  ...
  const removeContact = (id) => {
    ...
    if (window.confirm(`Do you want to delete ${contactToRemove.name}`)){
        ...
        .catch(error => {
          setErrorMessage(
            `ERROR: Could not remove ${contactToRemove.name} from the server`
          )
          setTimeout(() => {
            setErrorMessage(null)
          }, 5000)
